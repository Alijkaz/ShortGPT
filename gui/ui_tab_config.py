...
class ConfigUI(AbstractComponentUI):
    ...
    def save_keys(self, openai_key, eleven_key, pexels_key, gemini_key, openrouter_key):
        ...
        if (self.api_key_manager.get_api_key('ELEVENLABS_API_KEY') != eleven_key):
            ...
            return gr.update(value=openai_key),\
                ...
                gr.update(value=openrouter_key),\
        ...
        with gr.Tab("Config") as config_ui:
            with gr.Row():
                with gr.Column():
                    ...
                    with gr.Row():
                        openrouter_textbox = gr.Textbox(value=self.api_key_manager.get_api_key("OPENROUTER_API_KEY"), label=f"OPENROUTER API KEY", show_label=True, interactive=True, show_copy_button=True, type="password", scale=40)
                        show_openrouter_key = gr.Button("Show", size="sm", scale=1)
                    ...
                    save_button.click(self.verify_eleven_key, [eleven_labs_textbox, eleven_characters_remaining], [eleven_characters_remaining]).success(
                        self.save_keys, [openai_textbox, eleven_labs_textbox, pexels_textbox, gemini_textbox, openrouter_textbox], [openai_textbox, eleven_labs_textbox, pexels_textbox, gemini_textbox, openrouter_textbox, AssetComponentsUtils.voiceChoice(), AssetComponentsUtils.voiceChoiceTranslation()])
